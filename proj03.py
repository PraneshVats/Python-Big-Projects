...
while True:
    print(WELCOME_TEXT)
    user_int=input(MAIN_PROMPT)
    if user_int=='Y':
        print("Enter value for each of the following items or type 'N/A' if unknown")
        Loc=input(LOCATIONS_TEXT)
        Sqft=input(SQUARE_FOOTAGE_TEXT)
        maxpay=input(MAX_MONTHLY_PAYMENT_TEXT)
        Downpay=input(DOWN_PAYMENT_TEXT)
        AprilRate=input(APR_TEXT)
        Loc=float(Loc)
        Sqft=float(Sqft)
        maxpay=float(maxpay)
